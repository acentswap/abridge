...
import (
	...
	"github.com/acentswap/abridge/common"
...
func (b *Bridge) verifyContractAddress(tokenCfg *tokens.TokenConfig) error {
	...
	if tokenCfg.IsDelegateContract && !tokenCfg.IsacentswapAdapter && !b.IsSrc {
	...
	if !b.IsSrc && !tokenCfg.IsMappingTokenProxy {
		err := b.VerifyacentswapContractAddress(contractAddr)
		if err != nil {
			return fmt.Errorf("wrong acentswap contract address: %v, %w", contractAddr, err)
		}
	}
